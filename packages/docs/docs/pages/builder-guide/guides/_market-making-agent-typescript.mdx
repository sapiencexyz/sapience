...
:::note[Collateral]
On Ethereal, prediction market contracts use [wUSDe](https://explorer.ethereal.trade/address/0xB6fC4B1BFF391e5F6b4a3D2C7Bda1FeE3524692D) (Wrapped USDe). The SDK's `prepareForTrade` helper handles wrapping and approval automatically.
:::
...
On Ethereal, the native token is USDe but prediction market contracts expect [wUSDe](https://explorer.ethereal.trade/address/0xB6fC4B1BFF391e5F6b4a3D2C7Bda1FeE3524692D) (Wrapped USDe). The SDK provides `prepareForTrade` to handle this automatically:
...
The starter uses the SDK's `createCounterpartyBid` helper, which handles EIP-712 typed data construction and signing in one call:
...
// Create and sign a bid in one step
const bidPayload = await createCounterpartyBid({
  auction,                          // AuctionDetails from auction.started
  counterparty: account.address,
  counterpartyCollateral: parseEther(BID_AMOUNT),
...
// Submit to relayer
ws.send(JSON.stringify({ type: 'bid.submit', payload: bidPayload }));
```
...
- **Sizing bids dynamically** based on `auction.predictorCollateral` or market conditions
- **Filtering by market** using `auction.picks` to target specific conditions
- **Adding risk limits** like max total exposure or per-counterparty rate limits
