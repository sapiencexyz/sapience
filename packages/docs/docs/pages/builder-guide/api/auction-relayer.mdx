...
The relayer performs structural validation only on the bid signature format. The signature is verified on-chain when the predictor calls `mint()` — that is the authoritative gate.

### Step 3 — Predictor: select a bid and mint

Bids arrive on the predictor's connection as `auction.bids` messages. Pick one (the SDK ranks by `counterpartyCollateral` — higher is better for the predictor), then assemble an `AuctionRequestPayload` client-side and call `mint()` on the escrow contract. The on-chain mint never goes through the relayer.
...
The relayer accepts intent signatures from smart accounts via four verification paths, in order:
...
5. **ERC-1271 fallback**: when an RPC client is configured, the relayer calls `isValidSignature` on the predictor contract. Signatures that can't be verified offline land here; if `isValidSignature` reverts, the auction is still accepted as `unverified` (passthrough), since the on-chain `mint()` is the authoritative gate.
...
`auction.start` is rejected with one of the error codes below if any of these checks fail. The relayer returns the human-readable reason from the SDK in `auction.ack.payload.error`; the SDK's machine-readable `ValidationErrorCode` enum is in [`packages/sdk/auction/validation.ts`](https://github.com/sapiencexyz/sapience/blob/main/packages/sdk/auction/validation.ts).
...
`bid.submit` performs a lighter check: structural validation of fields (address shape, signature format, deadline freshness, auction existence). The full `MintApproval` signature is verified on-chain at mint time; the SDK exposes `validateBidOnChain()` and `simulateMint()` for callers that want pre-submit confidence.
