...
import { TimeWindow, Candle } from './interfaces';
import {
  formatDbBigInt,
...
const startServer = async () => {
  ...
  app.get(
    ...
    handleAsyncErrors(async (req, res) => {
      ...
      const chartData = groupedPrices.reduce<Candle[]>((acc: Candle[], group) => {
        ...
        // if we have prices, calculate HLOC normally and open from the last candle
        const lastCandle = acc[acc.length - 1];
