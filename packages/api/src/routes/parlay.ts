	...
	try {
		...
		for (let i = 0; i < markets.length; i++) {
			...
			if (marketExists.market_group) {
				const marketGroupId = marketExists.market_group.id;
				if (marketGroupIds.has(marketGroupId)) {
					return res.status(400).json({
						message: `Multiple markets from the same market group are not allowed (marketGroupId: ${marketGroupId})`,
					});
				}
				marketGroupIds.add(marketGroupId);
			}
		}
