...
export class CandleCacheBuilder extends BaseCandleCacheBuilder {
  ...
  public async buildAllCandles() {
    ...
    log({ message: 'step 2: process resource prices', prefix: CANDLE_CACHE_CONFIG.logPrefix });
    const lastProcessedResourcePrice = await getParam(CANDLE_CACHE_CONFIG.lastProcessedResourcePrice);
    await this.processResourcePrices(lastProcessedResourcePrice);
    
    log({ message: 'step 3: process market prices', prefix: CANDLE_CACHE_CONFIG.logPrefix });
    const lastProcessedMarketPrice = await getParam(CANDLE_CACHE_CONFIG.lastProcessedMarketPrice);
    await this.processMarketPrices(lastProcessedMarketPrice);
    
    log({ message: 'step 4: save all runtime candles', prefix: CANDLE_CACHE_CONFIG.logPrefix });
