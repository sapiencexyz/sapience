...
export abstract class BaseCandleCacheBuilder {
  ...
  protected getResourcePricesCountFn: (params: ResourcePriceParams) => Promise<number> = getResourcePricesCount;
  protected getResourcePricesFn: (params: ResourcePriceParams) => Promise<{ prices: any[]; hasMore: boolean }> = getResourcePrices;
  ...
  protected async processResourcePrices(initialTimestamp: number = 0) {
    log({ message: 'step 1: process resource prices', prefix: CANDLE_CACHE_CONFIG.logPrefix });
    
    let getNextBatch = true;
    
    const totalResourcePrices = await this.getResourcePricesCountFn({ initialTimestamp, quantity: 0 });
    ...
    while (getNextBatch) {
      ...
      if (prices.length === 0) {
        log({ message: `batch is empty: ${iter}/${totalBatches} - step 2`, prefix: CANDLE_CACHE_CONFIG.logPrefix, indent: 2 });
        break;
      }
      
      log({ message: `batch: ${iter}/${totalBatches} - step 2, process the batch of size: ${prices.length}`, prefix: CANDLE_CACHE_CONFIG.logPrefix, indent: 2 });
      ...
      while (batchIdx < prices.length) {
        ...
        for (const trailingAvgTime of CANDLE_CACHE_CONFIG.trailingAvgTime) {
          await this.trailingAvgCandleProcessor.processResourcePrice(price, trailingAvgTime);
        }
      ...
      await setParam(CANDLE_CACHE_CONFIG.lastProcessedResourcePrice, initialTimestamp);
    }
  }

  protected async processMarketPrices(initialTimestamp: number = 0) {
    log({ message: 'step 1: process market prices', prefix: CANDLE_CACHE_CONFIG.logPrefix });
    ...
    while (getNextBatch) {
      iter++;
      log({ message: `batch: ${iter}/${totalBatches} - step 1`, prefix: CANDLE_CACHE_CONFIG.logPrefix, indent: 2 });
      ...
      if (prices.length > 0) {
        initialTimestamp = prices[prices.length - 1].timestamp;
        await setParam(CANDLE_CACHE_CONFIG.lastProcessedMarketPrice, initialTimestamp);
      }
    }
  }
