...
describe('PR6 leaderboard', () => {
  ...
  it('returns ACCURACY rankings from scoring outputs without normalizing value', async () => {
    ...
    const result = await callResolver<{
      ...
      nodes: Array<{ account: { address: string }; rank: number; value: string }>;
    }>(leaderboard)(
      null,
    ...
    expect(result.nodes).toEqual([
      { account: expect.objectContaining({ address: '0xabc' }), rank: 1, value: '0.91' },
      { account: expect.objectContaining({ address: '0xdef' }), rank: 2, value: '0.5' },
    ]);
  });

  it('uses account-stat volume as positive wUSDe value', async () => {
    vi.mocked(getMerged).mockResolvedValue([
      { address: '0xaaa', netPnL: '10', gains: '10', losses: '0', volume: '25' },
    ]);
  ...
  it('derives ROI as netPnL divided by volume and returns zero for zero volume', async () => {
    vi.mocked(getMerged).mockResolvedValue([
      { address: '0xaaa', netPnL: '50', gains: '50', losses: '0', volume: '100' },
      { address: '0xbbb', netPnL: '50', gains: '50', losses: '0', volume: '0' },
    ]);

    const result = await callResolver<{ nodes: Array<{ value: string }> }>(
      leaderboard
    )(null, { metric: LeaderboardMetric.Roi, first: 10 }, {}, null);

    expect(result.nodes.map((n) => n.value)).toEqual(['0.5', '0']);
  });
});
