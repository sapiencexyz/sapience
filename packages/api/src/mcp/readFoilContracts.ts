...
const formatMarketParams = (params: any): MarketParams => {
  return {
...
export const getMarketGroupInfo = {
  ...
  function: async (args: { marketGroupAddress: string }) => {
    try {
      ...
      }) as readonly [ `0x${string}`, `0x${string}`, `0x${string}`, `0x${string}`, MarketParams ];
    ...
    try {
      ...
      const formattedInfo: MarketDetailsData = {
        marketData: marketDetailsResult[0] ? {
          ...
          baseAssetMinPriceTick: marketDetailsResult[0].baseAssetMinPriceTick,
          baseAssetMaxPriceTick: marketDetailsResult[0].baseAssetMaxPriceTick,
        ...
        marketGroupParams: marketDetailsResult[1] ? formatMarketParams(marketDetailsResult[1]) : null
      };
