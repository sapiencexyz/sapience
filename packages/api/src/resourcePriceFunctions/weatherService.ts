...
export default class WeatherService {
     ...
     */
    public async getHistoricalData(startTimestamp: number, endTimestamp?: number): Promise<WeatherSummary[]> {
        const { startDate, endDate } = this.getDateRange(startTimestamp, endTimestamp);
        ...
        return this.processHistoricalData(tempRecords, precipRecords);
    }

    /**
     * Fetches precipitation data from NOAA API for Los Angeles
     */
    private async fetchLAPrecipitation(startDate: Date, endDate: Date): Promise<WeatherRecord[]> {
        try {
     ...
     */
    private async fetchNYCTemperature(startDate: Date, endDate: Date): Promise<WeatherRecord[]> {
        try {
    ...
    private async checkForUpdates(): Promise<void> {
        try {
            ...
            const [tempRecords, precipRecords] = await Promise.all([
                this.fetchNYCTemperature(new Date(Date.now() - CONFIG.LOOKBACK_DAYS * 24 * 60 * 60 * 1000), new Date()),
     ...
     */
    private getCurrentSummary(): WeatherSummary {
        return {
            timestamp: new Date().toISOString(),
            nycStation: this.nycStation,
            laStation: CONFIG.SF.NAME,
     ...
     */
    private processHistoricalData(tempRecords: WeatherRecord[], precipRecords: WeatherRecord[]): WeatherSummary[] {
        ...
        const allTimestamps = [...new Set([...tempMap.keys(), ...precipMap.keys()])].sort();
    ...
    private getDateRange(startTimestamp: number, endTimestamp?: number): { startDate: Date; endDate: Date } {
