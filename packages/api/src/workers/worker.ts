...
async function main() {
  ...
  for (const resourceInfo of RESOURCES) {
    ...
    if (resourceInfo.priceIndexer) {
      ...
      console.log(
        `[Worker] Starting initial indexing for resource ${resourceInfo.name}`
      );
      try {
        await resourceInfo.priceIndexer.indexBlocks(resource, []);
        console.log(
          `[Worker] Completed initial indexing for resource ${resourceInfo.name}`
        );
      } catch (error) {
        console.error(
          `[Worker] Error during initial indexing for ${resourceInfo.name}:`,
          error
        );
      }
