...
describe('SecondaryMarketIndexer', () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    mockPrisma.v2SecondaryTrade.upsert.mockResolvedValue({});
    mockPrisma.v2SecondaryIndexerState.upsert.mockResolvedValue({});
    mockPrisma.v2SecondaryIndexerState.findUnique.mockResolvedValue(null);
  ...
  it('should decode TradeExecuted event and upsert trade', async () => {
    ...
    expect(mockPrisma.v2SecondaryTrade.upsert).toHaveBeenCalledTimes(1);
    const call = mockPrisma.v2SecondaryTrade.upsert.mock.calls[0][0];
  ...
  it('should be idempotent — same tradeHash upserted twice without error', async () => {
    ...
    expect(mockPrisma.v2SecondaryTrade.upsert).toHaveBeenCalledTimes(2);
    // Both calls use the same tradeHash — upsert handles idempotency
    const hash1 = mockPrisma.v2SecondaryTrade.upsert.mock.calls[0][0].where.tradeHash;
    const hash2 = mockPrisma.v2SecondaryTrade.upsert.mock.calls[1][0].where.tradeHash;
  ...
  it('should update indexer state checkpoint during watchBlocksForResource', async () => {
    ...
    expect(mockPrisma.v2SecondaryIndexerState.upsert).toHaveBeenCalledTimes(1);
    const call = mockPrisma.v2SecondaryIndexerState.upsert.mock.calls[0][0];
