...
import { Log, decodeEventLog, Abi, PublicClient } from 'viem';
 ...
 */
async function startIndexingForMarketGroup(marketGroup: MarketGroup, client: PublicClient) {
    console.log(`Starting event indexing for Market Group: ${marketGroup.chainId}:${marketGroup.address}`);
    try {
        await indexMarketEvents(marketGroup, client); 
    } catch (error) {
        console.error(`Error starting indexing for market group ${marketGroup.address}:`, error);
    }
}

async function handleMarketGroupInitialized(
    eventArgs: { marketGroup: `0x${string}`, returnData: `0x${string}`, nonce: bigint },
    chainId: number,
    ...
    try {
        ...
        if (marketGroupRecord) {
            console.log(`Found market group with nonce ${nonce} and chainId ${chainId}, updating address to ${newMarketGroupAddress}`);
            ...
            console.log(`Market group with nonce ${nonce} and chainId ${chainId} not found. Creating new market group.`);
            ...
            console.log(`New market group created successfully with ID ${marketGroupRecord.id}.`);
        ...
        console.error(`Error processing MarketGroupInitialized event for nonce ${nonce} on chain ${chainId}:`, error);
    }
}
...
export async function startIndexingAndWatchingMarketGroups(chainId: number) {
    ...
    const marketGroups = await marketGroupRepository.find({
        ...
        select: ['id', 'address', 'factoryAddress', 'initializationNonce', 'chainId'],
        relations: ['marketParams'], // Add marketParams relation
    });

    console.log(`Found ${marketGroups.length} existing market groups for chainId ${chainId}. Starting indexing...`);
    ...
    for (const marketGroup of marketGroups) {
        ...
        if (marketGroup.address && marketGroup.address !== '0x0000000000000000000000000000000000000000') {
    ...
    const factoryAddresses = [
        ...new Set(marketGroups.map((mg) => mg.factoryAddress).filter((addr): addr is string => !!addr)),
    ];
    console.log(`Found ${factoryAddresses.length} unique factory addresses for chainId ${chainId}. Setting up watchers...`);

    // Watch for MarketGroupInitialized events on each factory
    factoryAddresses.forEach((factoryAddress) => {
        console.log(`Watching MarketGroupInitialized events on factory ${factoryAddress}`);
            ...
            onLogs: (logs: Log[]) => {
                logs.forEach((log) => {
                    try {
                        ...
                        handleMarketGroupInitialized(eventArgs, chainId, factoryAddress, client); // Pass client argument
                    } catch (error) {
                        console.error(`Error decoding MarketGroupInitialized event from factory ${factoryAddress}:`, error, log);
                    }
                });
            },
            onError: (error) => {
                console.error(`Error watching MarketGroupInitialized events on factory ${factoryAddress}:`, error);
            },
        });
    });

    console.log(`Initialization and watching setup complete for chainId ${chainId}.`);
}
