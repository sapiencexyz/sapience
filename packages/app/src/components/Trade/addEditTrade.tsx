...
export default function AddEditTrade() {
  ...
  // Trade state management with validated market data
  const {
    form,
    sizeChange,
    option,
    ...
    refetchAllowance,
    isLong: option === 'Long',
    quotedResultingWalletBalance: '0',
    walletBalance: '0',
  ...
  // Trade calculations with validated market data
  const {
    ...
    option,
    positionData,
    collateralBalance: address ? BigInt(0) : undefined, // You'll need to get actual balance
    collateralAssetDecimals,
    marketAddress,
    epoch: epoch ?? 0,
  ...
  const requireApproval = !allowance || (quotedCollateralDelta > (allowance as bigint));

  // Check if quote is being generated
  const isGeneratingQuote = isEdit
    ? form.formState.isValidating
    : form.formState.isSubmitting;

  if (!collateralAsset || !collateralAssetTicker || !collateralAssetDecimals) {
    return (
      <div className="p-4 text-center">
        <p>Loading collateral asset data...</p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <TradeActions
        ...
        closePositionPriceImpact={closePositionPriceImpact}
        sizeChangeInContractUnit={sizeChangeInContractUnit}
        desiredSizeInContractUnit={desiredSizeInContractUnit}
        fetchingSizeFromCollateralInput={form.watch('fetchingSizeFromCollateralInput')}
        isLoadingCollateralChange={isLoadingCollateralChange}
        isNonZeroSizeChange={isNonZeroSizeChange}
        requireApproval={requireApproval}
        collateralAssetTicker={collateralAssetTicker}
        isGeneratingQuote={isGeneratingQuote}
        onConnectWallet={() => setIsOpen(true)}
        onSwitchNetwork={() => switchChain({ chainId })}
        setValue={(key, value) => form.setValue(key as keyof TradeFormValues, value as any)}
      />
      <PositionInfo
        isEdit={isEdit}
        positionData={positionData}
        ...
        originalPositionSizeInContractUnit={originalPositionSizeInContractUnit}
        desiredSizeInContractUnit={desiredSizeInContractUnit}
        walletBalance={walletBalance}
        quotedResultingWalletBalance={quotedResultingWalletBalance}
        walletBalanceLimit={walletBalanceLimit}
        positionCollateralLimit={positionCollateralLimit}
