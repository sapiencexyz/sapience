...
import type { V2AuctionDetails, V2ValidatedBid } from '@sapience/sdk/types/v2';
...
type Props = {
  ...
  auction: V2AuctionDetails | null;
  collateralSymbol: string;
};

/**
 * Hook to subscribe to V2 auction bids via WebSocket
 */
function useV2AuctionBids(auctionId: string | null) {
  const [bids, setBids] = useState<V2ValidatedBid[]>([]);
...
const V2AuctionBidsDialog: React.FC<Props> = ({
  auctionId,
  auction,
  collateralSymbol,
}) => {
  const [open, setOpen] = useState(false);
  const { bids } = useV2AuctionBids(auctionId);
...
export default V2AuctionBidsDialog;
