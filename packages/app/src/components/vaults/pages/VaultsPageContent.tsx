...
const VaultsPageContent = () => {
  ...
  const yieldMetrics = useMemo(() => {
    ...
    // Protocol TVL = protocol-wide escrow + undeployed assets summed across every vault.
    // Escrow comes from protocolStats so the vault-specific API doesn't pretend a
    // chain-wide escrow balance belongs to any one vault.
    const latestProtocolStat = protocolStats?.[protocolStats.length - 1];
    const escrowWei = latestProtocolStat?.escrowBalance
      ? BigInt(latestProtocolStat.escrowBalance)
      : 0n;
