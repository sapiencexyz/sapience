...
const MarketListContext = createContext<MarketListContextType | undefined>(undefined);
...
export const MarketListProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const { data: markets, isLoading, error } = useQuery<Market[], Error>({
    ...
    queryFn: async () => {
      ...
      return fetchedMarkets.map(market => {
        const sortedEpochs = [...market.epochs].sort((a, b) => a.startTimestamp - b.startTimestamp);
        
        const currentEpoch = sortedEpochs.find(epoch => 
          epoch.startTimestamp <= currentTimestamp && epoch.endTimestamp > currentTimestamp
        ) || sortedEpochs[sortedEpochs.length - 1] || null;
        
        const nextEpoch = sortedEpochs.find(epoch => 
  ...
  return (
    <MarketListContext.Provider value={{ markets: markets || [], isLoading, error }}>
      {children}
