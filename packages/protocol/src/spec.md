...
- Mint accepts EOA (ECDSA) signatures and EIP-1271 (smart-account `isValidSignature`) signatures
- Smart accounts using session keys must authorize the session key through their own validator (e.g. Kernel permission validator) so `isValidSignature` returns the magic value when the session key signs
...
- MintRequest includes: picks, two addresses, two collateral amounts, two signatures, two nonces, two deadlines, refCode, optional predictor sponsor + sponsor data
- It validates the signatures (EOA ECDSA or smart account ERC-1271 via SignatureValidator)
...
- Both parties must sign via EIP-712 (EOA ECDSA or smart account ERC-1271)
...
6. Verify both signatures (predictor holder and counterparty holder) — supports EOA ECDSA and smart account ERC-1271
7. Validate and consume bitmap nonces for both parties
8. Burn predictor tokens from predictor holder
9. Burn counterparty tokens from counterparty holder
10. Update accounting: decrease `totalPredictorCollateral` and `totalCounterpartyCollateral`
11. Transfer collateral payouts to both holders
12. Emit `PositionsBurned` event
...
- `setTrustedMintRouter(router)`: Designate the CommittedIntentExecutor that may bypass per-party MintApproval signatures (owner only)
