...
#### Token Supply = Collateral Amount (1:1 Ratio)
...
- User A: predictor, deposited 50 USDE, owns 50 predictor tokens
- User B: predictor, deposited 30 USDE, owns 30 predictor tokens (SAME token as User A)
- User C: counterparty, deposited 40 USDE, owns 40 counterparty tokens
- User D: counterparty, deposited 60 USDE, owns 60 counterparty tokens
...
### Mint process
- Mint function receives picks, two addresses, two collateral amounts, two sets of signatures, two sets of nonces and deadlines
...
- It transfers the collateral from both addresses to escrow. `PredictionCollateral = PredictorCollateral + CounterpartyCollateral`
- It creates or reuses conditional position tokens:
  ...
  - Mints tokens proportional to collateral (1:1 ratio): predictor gets PredictorCollateral tokens, counterparty gets CounterpartyCollateral tokens
...
- The holder of any position token burns their tokens and receives proportional payout:
  ...
  - originalTotalTokens = total collateral deposited for that side (doesn't change as tokens are burned)
...
- Create prediction (mint):
  ...
  - Mint tokens proportional to collateral (1:1 ratio)
- Additional predictions (same picks):
  - Same pick configuration reuses existing tokens
  - New tokens minted proportional to new collateral, added to the pool
