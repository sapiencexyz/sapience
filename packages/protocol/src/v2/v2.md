...
- **Ethereal** (source): `_processedBridgeBacks[bridgeId]` prevents double escrow release
...
4. If no ACK received (message lost/failed):
   - Anyone calls retryBridge(bridgeId) on Ethereal (permissionless)
...
5. Emergency escape (after 7 days without ACK):
   - Only original sender calls emergencyCancelBridge(bridgeId)
...
4. If no ACK received (message lost/failed):
   - Anyone calls retryBridgeBack(bridgeId) on Arbitrum (permissionless)
   - Must wait MIN_RETRY_DELAY (5 minutes) between retries
   - Resends the same message with same bridgeId
   - Ethereal processes idempotently (safe to retry)

5. Emergency escape (after 7 days without ACK):
   - Only original sender calls emergencyCancelBridgeBack(bridgeId)
   - Escrowed tokens returned to original sender
...
- `isBridgeProcessed(bridgeId)`: Check if bridge was processed on Arbitrum (mint idempotency)
- `isBridgeBackProcessed(bridgeId)`: Check if bridgeBack was processed on Ethereal (release idempotency)
