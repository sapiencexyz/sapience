...
- `PositionTokenBridge.sol`: Ethereal side - escrow, send, receive ACK, release
...
A passive liquidity vault that allows users to deposit assets and earn yield through EOA-managed protocol interactions. Simplified from V1 to work with PredictionMarketV2's ERC20 position tokens.

#### Overview

The vault enables liquidity providers to pool assets that a designated manager can deploy to PredictionMarketV2 for counterparty liquidity. Users receive vault shares representing their proportional ownership.
