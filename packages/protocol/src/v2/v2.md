...
The bridge uses idempotent processing to prevent race conditions and ensure consistency:

- Remote side tracks processed bridges via `_processedBridges[bridgeId]`
- If a retry arrives after processing: skip mint, just re-send ACK
- No possibility of double-minting or unbacked tokens
- Users can safely retry without risk of accounting errors
...
4. If no ACK received (message lost/failed):
   - User calls retryBridge(bridgeId) on Ethereal
   - Must wait MIN_RETRY_DELAY (5 minutes) between retries
   - Resends the same message with same bridgeId
   - Remote processes idempotently (safe to retry)

5. Emergency escape (after 7 days without ACK):
   - User calls emergencyCancelBridge(bridgeId)
   - Escrowed tokens returned to original sender
   - Cancel notification sent to Arbitrum
   - Status: CANCELLED
```
...
1. User calls bridgeBack(token, recipient, amount) on Arbitrum:
   ...
   - PendingBridgeBack record created with unique bridgeId
...
3. Arbitrum receives ACK (_lzReceive):
   ...
   - Mark bridge as COMPLETED
   - Emit BridgeBackCompleted event

4. If no ACK received (message lost/failed):
   - User calls retryBridgeBack(bridgeId) on Arbitrum
   - Must wait MIN_RETRY_DELAY (5 minutes) between retries
   - Resends the same message with same bridgeId
   - Ethereal processes idempotently (safe to retry)

5. Emergency escape (after 7 days without ACK):
   - User calls emergencyCancelBridgeBack(bridgeId)
   - Escrowed tokens returned to original sender
...
- `CANCELLED`: Emergency cancelled after 7 days (tokens returned)

#### Recovery Mechanisms

| Scenario | Solution | Risk |
|----------|----------|------|
| ACK never arrives | Retry with `retryBridge()` or `retryBridgeBack()` | None - idempotent |
| Remote chain down | Wait and retry when available | None - idempotent |
| Extended failure (7+ days) | Emergency cancel to recover tokens | Cancel is emergency only |

#### Time Constants
- `MIN_RETRY_DELAY`: 5 minutes between retry attempts
- `EMERGENCY_CANCEL_DELAY`: 7 days before cancel is allowed

#### View Functions
- `getPendingBridges(sender)` / `getPendingBridgeBacks(sender)`: Get pending bridge IDs
- `isBridgeProcessed(bridgeId)`: Check if bridge was already processed (for idempotency)
- `getMinRetryDelay()` / `getEmergencyCancelDelay()`: Get time constants
...
- `CMD_CANCEL (4)`: Emergency cancel notification (7 days)
