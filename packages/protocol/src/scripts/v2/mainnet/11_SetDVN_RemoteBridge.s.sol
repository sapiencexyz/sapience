...
/// @title Set DVN Configuration for SM Network Bridge (Mainnet)
/// @notice Configures SendLib, ReceiveLib, DVN, and Executor for the Arbitrum mainnet bridge
contract SetDVN_RemoteBridge is Script {
    ...
    function run() external {
        ...
        address dvn2 = vm.envAddress("SM_NETWORK_DVN_2");
        address executor = vm.envAddress("SM_NETWORK_EXECUTOR");

        console.log("=== Set DVN for SM Network Bridge (Mainnet) ===");
        ...
        // 3. Set receive ULN config (DVN for incoming messages from PM Network)
        _setReceiveConfig(endpoint, bridge, remoteEid, receiveLib, dvn1, dvn2);
        console.log("Receive DVN config set (2 DVNs)");

        // 4. Set send ULN config (DVN and executor for outgoing messages to PM Network)
        _setSendConfig(
