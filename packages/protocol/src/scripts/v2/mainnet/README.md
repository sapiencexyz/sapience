# V2 Bridge Mainnet Deployment Scripts

Scripts for deploying and configuring the V2 position token bridge between PM Network (Ethereal) and SM Network (Arbitrum) mainnets.
...
forge script src/scripts/v2/mainnet/00_DeployCollateral.s.sol \
...
forge script src/scripts/v2/mainnet/01_DeployResolver.s.sol \
  --rpc-url $PM_NETWORK_RPC_URL \
  --broadcast \
  --verify --verifier blockscout --verifier-url https://explorer.ethereal.trade/api/ \
  -vvvv
...
forge script src/scripts/v2/mainnet/03_DeployEtherealBridge.s.sol \
  --rpc-url $PM_NETWORK_RPC_URL \
  --broadcast \
  --verify --verifier blockscout --verifier-url https://explorer.ethereal.trade/api/ \
  -vvvv
...
forge script src/scripts/v2/mainnet/04_DeployFactory.s.sol \
...
forge script src/scripts/v2/mainnet/05_DeployRemoteBridge.s.sol \
  --rpc-url $SM_NETWORK_RPC_URL \
  --broadcast \
  --verify --etherscan-api-key $SM_NETWORK_ETHERSCAN_API_KEY \
  -vvvv
...
forge script src/scripts/v2/mainnet/06_ConfigureEtherealBridge.s.sol --rpc-url $PM_NETWORK_RPC_URL --broadcast -vvvv

# 6b. Set DVN/Libraries for Ethereal Bridge
forge script src/scripts/v2/mainnet/06b_SetDVN_EtherealBridge.s.sol --rpc-url $PM_NETWORK_RPC_URL --broadcast -vvvv

# 7. Configure Remote Bridge (run on SM Network)
forge script src/scripts/v2/mainnet/07_ConfigureRemoteBridge.s.sol --rpc-url $SM_NETWORK_RPC_URL --broadcast -vvvv

# 7b. Set DVN/Libraries for Remote Bridge
forge script src/scripts/v2/mainnet/07b_SetDVN_RemoteBridge.s.sol --rpc-url $SM_NETWORK_RPC_URL --broadcast -vvvv
```
...
forge script src/scripts/v2/mainnet/08a_CheckStatus_PMNetwork.s.sol --rpc-url $PM_NETWORK_RPC_URL -vvvv

# 8b. Check SM Network status
forge script src/scripts/v2/mainnet/08b_CheckStatus_SMNetwork.s.sol --rpc-url $SM_NETWORK_RPC_URL -vvvv
```
