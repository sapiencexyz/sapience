...
**Optional env vars:**
- `VAULT_NAME` - Token name (default: "Foil Vault V2")
...
```bash
forge script src/scripts/v2/mainnet/vault/02_TestDepositWithdrawal.s.sol \
  --rpc-url $PM_NETWORK_RPC_URL \
  --broadcast \
  -vvvv
```
...
```bash
forge script src/scripts/v2/mainnet/vault/03_TestVaultAsCounterparty.s.sol \
  --rpc-url $PM_NETWORK_RPC_URL \
  --broadcast \
  -vvvv
```
