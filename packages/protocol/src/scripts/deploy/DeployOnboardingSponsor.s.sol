...
/// @title Deploy OnboardingSponsor
/// @notice Deploys an OnboardingSponsor for the onboarding flow
///
/// Prerequisites:
///   PredictionMarketVault must be deployed first (VAULT_ADDRESS in .env)
///
/// Required env vars:
...
///   REQUIRED_COUNTERPARTY           - required counterparty address (defaults to VAULT_ADDRESS)
...
contract DeployOnboardingSponsor is Script {
    function run() external {
        ...
        address requiredCounterparty =
            vm.envOr("REQUIRED_COUNTERPARTY", vaultAddress);
        require(
            requiredCounterparty != address(0),
            "Set REQUIRED_COUNTERPARTY or VAULT_ADDRESS in .env"
        );
        uint256 maxEntryPriceBps = vm.envUint("MAX_ENTRY_PRICE_BPS");
        uint256 matchLimit = vm.envOr("MATCH_LIMIT", uint256(1 ether));

        console.log("=== Deploy OnboardingSponsor ===");
        ...
        console.log("ONBOARDING_SPONSOR_ADDRESS=", address(sponsor));
        console.log("");
