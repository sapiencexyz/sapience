# PredictionMarketVault Scripts

Scripts for deploying and testing PredictionMarketVault on Ethereal.
...
```bash
forge script src/scripts/deploy/vault/01_DeployVault.s.sol \
  --rpc-url $PM_NETWORK_RPC_URL \
  --broadcast \
...
- `VAULT_NAME` - Token name (default: "Sapience Vault V2")
...
```bash
forge script src/scripts/deploy/vault/02_TestDepositWithdrawal.s.sol \
  --rpc-url $PM_NETWORK_RPC_URL \
  --broadcast \
  -vvvv
```
...
```bash
forge script src/scripts/deploy/vault/03_TestVaultAsCounterparty.s.sol \
  --rpc-url $PM_NETWORK_RPC_URL \
  --broadcast \
  -vvvv
```
