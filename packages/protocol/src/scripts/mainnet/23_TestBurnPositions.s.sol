...
contract TestBurnPositions is Script {
    ...
    function _setup() internal view returns (BurnConfig memory c) {
        ...
        IV2Types.PickConfiguration memory config = market.getPickConfiguration(c.pickConfigId);
        uint256 pBacking = config.totalPredictorTokensMinted > 0
            ? (c.burnAmount * config.totalPredictorCollateral) / config.totalPredictorTokensMinted
            : 0;
        uint256 cBacking = config.totalCounterpartyTokensMinted > 0
            ? (c.burnAmount * config.totalCounterpartyCollateral) / config.totalCounterpartyTokensMinted
            : 0;
    ...
    function _buildSignedRequest(BurnConfig memory c) internal view returns (IV2Types.BurnRequest memory) {
        uint256 pNonce = uint256(keccak256(abi.encode(block.timestamp, "bp", block.prevrandao)));
        uint256 cNonce = uint256(keccak256(abi.encode(block.timestamp, "bc", block.prevrandao)));
        uint256 deadline = block.timestamp + 1 hours;
        bytes32 burnHash = _computeBurnHash(c);

        bytes memory pSig = _sign(c, SignInput(burnHash, c.burnAmount, c.totalPayout, pNonce, deadline));
        bytes memory cSig = _sign(c, SignInput(burnHash, c.burnAmount, 0, cNonce, deadline));
    ...
    function _computeBurnHash(BurnConfig memory c) internal pure returns (bytes32) {
        return keccak256(
            abi.encode(c.pickConfigId, c.burnAmount, c.burnAmount, c.holder, c.holder, c.totalPayout, uint256(0))
        );
    }

    function _sign(BurnConfig memory c, SignInput memory s) internal view returns (bytes memory) {
        bytes32 h = PredictionMarketEscrow(MARKET).getBurnApprovalHash(
            s.burnHash, c.holder, s.tokenAmount, s.payout, s.nonce, s.deadline
        );
    ...
    function _logBefore(BurnConfig memory c) internal view {
        ...
        console.log("");
        console.log("--- Before ---");
        console.log("Predictor tokens:", IERC20(c.predictorToken).balanceOf(c.holder));
        console.log("Counterparty tokens:", IERC20(c.counterpartyToken).balanceOf(c.holder));
