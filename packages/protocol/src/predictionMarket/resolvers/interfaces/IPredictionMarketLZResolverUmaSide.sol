...
interface IPredictionMarketLZResolverUmaSide is ILayerZeroBridge {
    ...
    event ConfigUpdated(
        address indexed bondCurrency, uint256 bondAmount, uint64 assertionLiveness, address indexed updater
    );
    ...
    event MarketSubmittedToUMA(
        bytes32 indexed marketId, bytes32 indexed assertionId, address asserter, bytes claim, bool resolvedToYes
    );
    event MarketResolvedFromUMA(
        bytes32 indexed marketId, bytes32 indexed assertionId, bool resolvedToYes, bool assertedTruthfully
    );
    event MarketDisputedFromUMA(bytes32 indexed marketId, bytes32 indexed assertionId);
