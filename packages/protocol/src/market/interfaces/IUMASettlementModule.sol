...
interface IUMASettlementModule {
    event SettlementSubmitted(
        uint256 marketId, address asserter, uint160 settlementSqrtPriceX96, uint256 submissionTime
    );
    ...
    function submitSettlementPrice(ISapienceStructs.SettlementPriceParams memory params) external returns (bytes32);
