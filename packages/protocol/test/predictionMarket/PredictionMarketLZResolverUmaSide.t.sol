...
import { PredictionMarketLZResolverUmaSide } from
    "../../src/predictionMarket/resolvers/PredictionMarketLZResolverUmaSide.sol";
import { IPredictionMarketLZResolverUmaSide } from
    "../../src/predictionMarket/resolvers/interfaces/IPredictionMarketLZResolverUmaSide.sol";
import { Encoder } from "../../src/bridge/cmdEncoder.sol";
import { BridgeTypes } from "../../src/bridge/BridgeTypes.sol";
import { MockOptimisticOracleV3ForPMResolver } from
    "./mocks/MockOptimisticOracleV3ForPMResolver.sol";
...
contract PredictionMarketLZResolverUmaSideTest is TestHelperOz5 {
    ...
    function test_constructor_validParameters() public view {
        ...
        assertEq(
            umaResolver.getOptimisticOracleV3(),
            address(mockOptimisticOracleV3),
            "Oracle should be set"
        );
        (address bondCurrencyAddr, uint256 bondAmt, uint64 liveness) =
            umaResolver.config();
        assertEq(
            bondCurrencyAddr,
            address(bondCurrency),
            "Bond currency should be set"
        );
        assertEq(bondAmt, BOND_AMOUNT, "Bond amount should be set");
        assertEq(
            liveness, ASSERTION_LIVENESS, "Assertion liveness should be set"
        );
    }
    ...
    function test_setBridgeConfig() public {
        ...
        BridgeTypes.BridgeConfig memory retrievedConfig =
            umaResolver.getBridgeConfig();
        assertEq(retrievedConfig.remoteEid, 999, "Remote EID should be updated");
        assertEq(
            retrievedConfig.remoteBridge,
            address(0x1234),
            "Remote bridge should be updated"
        );
    }

    function test_setConfig() public {
        address newBondCurrency =
            address(0x1111111111111111111111111111111111111112);
        PredictionMarketLZResolverUmaSide.Settings memory newConfig =
        PredictionMarketLZResolverUmaSide.Settings({
        ...
        (address bondCurrencyAddr, uint256 bondAmt, uint64 liveness) =
            umaResolver.config();
        assertEq(
            bondCurrencyAddr, newBondCurrency, "Bond currency should be updated"
        );
    ...
    function test_approveAsserter() public {
        ...
        assertTrue(
            umaResolver.isAsserterApproved(newAsserter),
            "New asserter should be approved"
        );
    }

    function test_revokeAsserter() public {
        assertTrue(
            umaResolver.isAsserterApproved(asserter),
            "Asserter should be approved initially"
        );
    ...
    function test_withdrawBond() public {
        ...
        assertEq(
            ...
            amount,
            "Balance should increase by withdrawal amount"
        );
    }
    ...
    function test_submitAssertion_success() public {
        ...
        assertEq(
            mappedMarketId, marketId, "Market ID mapping should be correct"
        );
    ...
    function test_submitAssertion_marketNotEnded() public {
        ...
        vm.prank(asserter);
        vm.expectRevert(
            IPredictionMarketLZResolverUmaSide.MarketNotEnded.selector
        );
        umaResolver.submitAssertion(TEST_CLAIM, TEST_END_TIME, true);
    }
    ...
    function test_submitAssertion_insufficientBond() public {
        ...
        PredictionMarketLZResolverUmaSide newResolver =
    ...
    function test_assertionResolvedCallback_success() public {
        ...
        bytes32 retrievedMarketId =
            umaResolver.getAssertionMarketId(assertionId);
        assertEq(retrievedMarketId, bytes32(0), "Market ID should be cleared");

        bytes32 retrievedAssertionId =
            umaResolver.getMarketAssertionId(marketId);
        assertEq(
            retrievedAssertionId, bytes32(0), "Assertion ID should be cleared"
        );
    }
    ...
    function test_assertionResolvedCallback_invalidAssertionId() public {
        ...
        vm.prank(address(mockOptimisticOracleV3));
        vm.expectRevert(
            IPredictionMarketLZResolverUmaSide.InvalidAssertionId.selector
        );
    ...
    function test_assertionDisputedCallback() public {
        ...
        bytes32 retrievedAssertionId =
            umaResolver.getMarketAssertionId(marketId);
        assertEq(
            retrievedAssertionId, assertionId, "Assertion ID should remain"
        );
    }
    ...
    function test_getMarketAssertionId() public view {
        ...
        assertEq(
            assertionId,
            bytes32(0),
            "Should return zero for non-existent market"
        );
    }

    function test_getAssertionMarketId() public view {
        bytes32 marketIdResult =
            umaResolver.getAssertionMarketId(keccak256("invalid"));
        assertEq(
            marketIdResult,
            bytes32(0),
            "Should return zero for non-existent assertion"
        );
    }
    ...
    function test_getConfig() public view {
        ...
        assertEq(
            bondCurrencyAddr,
            address(bondCurrency),
            "Bond currency should match"
        );
        assertEq(bondAmt, BOND_AMOUNT, "Bond amount should match");
        assertEq(
            liveness, ASSERTION_LIVENESS, "Assertion liveness should match"
        );
    }
