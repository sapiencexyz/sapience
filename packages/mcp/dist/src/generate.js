...
async function execInProtocolDir(command) {
...
async function generateAST(filePath) {
    try {
        ...
        const outputPath = path.join('../protocol/out', contractName + '.sol', contractName + '.json');
...
function solidityToTypeScript(solidityType) {
    // Handle array types
...
function formatParamValue(param, name) {
...
function generateFunctionArgs(params) {
    return params.length > 0
        ? `args: [${params.map(p => formatParamValue(p, p.name === 'data' ? 'dataParam' : p.name)).join(', ')}],\n`
        : '';
}
// Helper function to generate return value handling
function generateReturnValue(returns) {
    if (returns.length === 1) {
        return '        return { result };\n';
    }
    else if (returns.length > 1) {
        return `        return {\n${returns.map((r, i) => `          ${r.name}: (result as [${returns.map(r => solidityToTypeScript(r.type)).join(', ')}])[${i}],\n`).join('')}        };\n`;
    }
    else {
...
function generateMCPTool(interfaceDefinition, abiPath) {
...
function extractABIFromAST(ast) {
    const abi = [];
    function processNode(node) {
        if (node.nodeType === 'FunctionDefinition') {
            const inputs = node.parameters.parameters.map((p) => ({
                name: p.name,
                type: p.typeName.typeDescriptions.typeString,
                internalType: p.typeName.typeDescriptions.typeString
            }));
            const outputs = node.returnParameters.parameters.map((p) => ({
                name: p.name || '',
                type: p.typeName.typeDescriptions.typeString,
                internalType: p.typeName.typeDescriptions.typeString
            }));
