...
export const upsertIndexPriceFromResourcePrice = async (resourcePrice: ResourcePrice) => {
    ...
    const epochRepository = getRepository(Epoch);
    const resourcePriceRepository = getRepository(ResourcePrice);
    const indexPriceRepository = getRepository(IndexPrice);

    // Get the market associated with the resource price
    const market = await marketRepository.findOne({ where: { id: resourcePrice.market.id } });
    if (!market) {
