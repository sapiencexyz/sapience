...
1) Maker sends `auction.start` with `maker`, `wager`, `resolver`, `predictedOutcomes[]`, `makerNonce`.
...
3) Takers send `bid.submit` with `auctionId`, `taker`, `takerWager`, `takerDeadline`, `takerSignature`, `makerNonce`.
