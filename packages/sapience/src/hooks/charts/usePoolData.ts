...
const TICK_SPACING_DEFAULT = 60; // Placeholder: Replace with actual import or definition

import type { GraphTick, PoolData, BarChartTick } from '~/lib/utils/liquidityUtil';
...
const formatPrice = (
  ...
  // Example: Add currency symbol ($, ¢, etc.) based on token name/symbol
    if (symbol.toLowerCase().includes('usd') || symbol.toLowerCase().includes('dai')) {
...
export function usePoolData({
  ...
  const [processedPoolData, setProcessedPoolData] = useState<PoolData | undefined>();
  const [orderBookData, setOrderBookData] = useState<Omit<UsePoolDataReturn, 'poolData' | 'isLoading' | 'isError' | 'error'>>({
      asks: [],
      bids: [],
      lastPrice: null,
      spread: null,
  ...
  const ticks = useMemo(() => {
    ...
    if (
        baseAssetMinPriceTick === undefined || baseAssetMaxPriceTick === undefined ||
        isNaN(baseAssetMinPriceTick) || isNaN(baseAssetMaxPriceTick) ||
  ...
  const contracts = useMemo(() => {
    if (!poolAddress || poolAddress === '0x' || !chainId || ticks.length === 0) {
        return [];
    }
      ...
      .map((tickResult, idx) => {
        ...
        if (typedResult.status === 'success' && typedResult.result) {
          ...
          const liquidityNet = typedResult.result[1];
           ...
           if (!JSBI.EQ(JSBI.BigInt(liquidityNet.toString()), JSBI.BigInt(0))) { // Check if liquidityNet is non-zero using JSBI
               return {
  ...
  useEffect(() => {
    ...
    if (pool && graphTicks.length > 0 && tickSpacing > 0 && !isLoadingTicks && !isErrorTicks) {
        ...
        getFullPool(pool, graphTicks, tickSpacing)
            ...
            .catch((err) => {
                 ...
                 setHookError(err instanceof Error ? err : new Error('Failed to process pool data'));
    ...
    } else if (!isLoadingTicks && graphTicks.length === 0 && ticks.length > 0 && rawTickData) {
  ...
  }, [pool, graphTicks, tickSpacing, isLoadingTicks, isErrorTicks, ticks.length, rawTickData]); // Add dependencies
    ...
    useEffect(() => {
        ...
        if (currentTickIndex < 0) {
             ...
             if (processedTicks.length > 0 && pool.tickCurrent !== undefined) {
                 ...
                 if (nearestTickIdx !== -1) {
                     ...
                     setOrderBookData({ asks: [], bids: [], lastPrice: lastPriceFormatted, spread: null });
        ...
        const bids: OrderBookLevel[] = rawBids.map(tick => {
        ...
        const asks: OrderBookLevel[] = rawAsks.map(tick => {
        ...
        if (lowestAskPrice !== undefined && highestBidPrice !== undefined && lowestAskPrice > highestBidPrice) {
            ...
            // Handle one-sided book - spread is effectively from last price to nearest level
             const nearestLevelPrice = asks.length > 0 ? lowestAskPrice : highestBidPrice;
  ...
  const isLoading = isLoadingTicks || (!processedPoolData && !hookError && contracts.length > 0 && pool !== null); // Also check pool exists before declaring loading finished
