...
import type { OrderBookLevel } from '~/hooks/charts/usePoolData'; // Import type from hook
...
const OrderBookChart: React.FC<OrderBookChartProps> = ({
    ...
    const { pool, isLoading: isLoadingPool, isError: isErrorPool } = useUniswapPool(
  ...
  if (isLoading) {
    return (
      <div className={`w-full border rounded-md bg-background text-foreground flex items-center justify-center min-h-[200px] ${className}`}>
  ...
  if (isError) {
    return (
      <div className={`w-full border rounded-md bg-destructive/10 text-destructive-foreground flex items-center justify-center min-h-[200px] p-4 ${className}`}>
  ...
  if (asks.length === 0 && bids.length === 0 && !isLoading) {
      return (
         <div className={`w-full border rounded-md bg-background text-foreground flex items-center justify-center min-h-[200px] ${className}`}>
             <p className="text-sm text-muted-foreground">No liquidity data available for this range.</p>
         </div>
      )
  }
  ...
  return (
    <div className={`w-full border rounded-md bg-background text-foreground ${className}`}>
      {/* Header */}
      <div className="grid grid-cols-3 gap-4 text-xs text-muted-foreground py-2 px-2 border-b">
        ...
        <span className="text-right">size ({pool?.token0?.symbol ?? 'Base'})</span>
        <span className="text-right">total ({pool?.token0?.symbol ?? 'Base'})</span>
      </div>

      {/* Asks (Sell Orders) - Rendered bottom-up */}
      <div className="flex flex-col-reverse min-h-[100px]">
        {asks.map((ask, index) => (
          <OrderBookRow key={`ask-${ask.rawPrice}-${index}`} {...ask} type="ask" />
        ))}
      </div>
      ...
      <div className="flex flex-col min-h-[100px]">
        {bids.map((bid, index) => (
          <OrderBookRow key={`bid-${bid.rawPrice}-${index}`} {...bid} type="bid" />
        ))}
      </div>
    </div>
  );
