...
function computeInitializedTicks(
  ...
  for (let i = 0; i < numSurroundingTicks; i++) { // Using numSurroundingTicks to limit iterations
...
export async function getFullPool(
  ...
  tickSpacing: number
): Promise<PoolData> { // Return type adjusted

    if (!pool) {
        throw new Error("Pool object is required for getFullPool");
    }
     if (graphTicks.length === 0) {
         console.warn("No graph ticks provided to getFullPool. Returning empty ticks.");
         return {
             pool,
             ticks: [],
         }
  ...
  return {
    pool: pool, // Return the original pool object
