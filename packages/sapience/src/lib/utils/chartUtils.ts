...
export interface MultiMarketChartDataPoint {
  ...
  resourceClose?: number; // Keep resource if needed
}
...
export const processCandleData = (
    ...
    resourceCandleData: CandleType[] | null
): MultiMarketChartDataPoint[] => {
    // Use a record keyed by timestamp, holding partial new data points
    const combinedData: Record<number, Partial<MultiMarketChartDataPoint> & { markets: Record<string, number | undefined> }> = {};
                ...
                try {
                    ...
                    if (!isNaN(closeValue)) {
                       ...
                       combinedData[ts].markets[marketId] = closeValue;
                        ...
                        console.warn(`Invalid market close value encountered for market ${marketId}: ${candle.close} at timestamp ${ts}`);
                        combinedData[ts].markets[marketId] = undefined; // Explicitly mark as undefined
                    }
                } catch(e) {
                    console.warn(`Error parsing market close value for market ${marketId}: ${candle.close} at timestamp ${ts}`, e);
                    combinedData[ts].markets[marketId] = undefined; // Explicitly mark as undefined
                }
            });
        }
    ...
    // Process resource candles (remains similar, populating a separate property)
    if (resourceCandleData) {
        resourceCandleData.forEach(candle => {
             if (candle.timestamp == null) return;

            const ts = candle.timestamp;
            allTimestamps.add(ts);
            if (!combinedData[ts]) {
                 combinedData[ts] = { timestamp: ts, markets: {} }; // Initialize if only resource has this ts
            }
             if (!combinedData[ts].markets) { // Ensure markets obj exists
                combinedData[ts].markets = {};
             }
            ...
            try {
                 const closeValue = parseFloat(String(candle.close));
                 if (!isNaN(closeValue)) {
                    combinedData[ts]!.resourceClose = closeValue;
                 } else {
                    console.warn(`Invalid resource close value encountered: ${candle.close} at timestamp ${ts}`);
                 }
            } catch (e) {
                console.warn(`Error parsing resource close value: ${candle.close} at timestamp ${ts}`, e);
            }
        });
    }
    ...
    return sortedTimestamps.map(ts => {
        ...
        return {
            ...
            resourceClose: point?.resourceClose, // Remains undefined if not present
        };
    });
};
