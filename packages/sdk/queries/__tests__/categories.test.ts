...
describe('fetchCategories', () => {
  test('returns categories from valid response', async () => {
    ...
    mockGraphqlRequest.mockResolvedValue({
      categoriesPage: { items: categories, hasMore: false },
    });
  ...
  test('throws when items is not an array', async () => {
    mockGraphqlRequest.mockResolvedValue({
      categoriesPage: { items: 'not-an-array', hasMore: false },
    });
    await expect(fetchCategories()).rejects.toThrow(
      'Failed to fetch categories: Invalid response structure'
    );
  ...
  test('returns empty array when response has empty items', async () => {
    mockGraphqlRequest.mockResolvedValue({
      categoriesPage: { items: [], hasMore: false },
    });
    const result = await fetchCategories();
