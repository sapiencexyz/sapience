...
export async function verifyAuctionIntentSignature(params: {
  ...
  try {
    ...
    // Wrapped in try because kernel-wrapped signatures (smart account session
    // keys) are longer than 65 bytes and viem throws on the recover.
    ...
    if (recovered && recovered.toLowerCase() === predictorAddress) {
