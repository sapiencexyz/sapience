...
The SDK provides a unified, three-tier validation pipeline for auction
...
|          | Tier 1: Offline                                      | Tier 2: On-chain Reads                                             | Tier 3: Mint Simulation                                         |
| -------- | ---------------------------------------------------- | ------------------------------------------------------------------ | --------------------------------------------------------------- |
| **What** | Fields + picks + deadline + offline sig verification | `verifyMintPartySignature()` + `isNonceUsed()` + balance/allowance | `eth_call` with state overrides simulating the full `mint()` tx |
| **RPC**  | None required\*                                      | Configurable fail-open                                             | Required                                                        |
| **When** | On arrival                                           | On arrival                                                         | Pre-submit (after predictor signs)                              |
| **Who**  | All consumers, always                                | Terminal, market makers (NOT relayer)                              | App only (predictor's submit flow)                              |
...
| `simulateMint`       | 3    | Full `mint()` via `eth_call` with state overrides        | Yes    |
...
└─────────────────┘                  └──────┬───────┘
              ...
              Each consumer validates independently using SDK
                        ...
                        Tier 1+2       Tier 1+2       Tier 1+2
                            │
                            ▼
                    Predictor signs → Tier 3 simulation → sendCalls
