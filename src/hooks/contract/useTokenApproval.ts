...
import erc20ABI from '~/lib/sdk/queries/abis/erc20abi.json';
import { parseAmountToBigInt } from '~/lib/sdk';
  ...
  const approve = async () => {
    if (
      ...
      const missingParamsError = new Error(
        'Missing required parameters for token approval'
      );
      console.error('Error approving tokens:', missingParamsError);
